...
describe('differenceInCalendarDays', function () {
  it('returns the number of calendar days between the given dates', function () {
    var result = differenceInCalendarDays(new Date(2012, 6
    /* Jul */
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInCalendarDays(new Date(2011, 6
    /* Jul */
  ...
  it('accepts timestamps', function () {
    var result = differenceInCalendarDays(new Date(2014, 8
    /* Sep */
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInCalendarDays(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInCalendarDays(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });
  ...
  dstOnly("works across DST start & end in local timezone: ".concat(tz || '(unknown)'), function () {
...

    {
      ...
      assert(sameTime(_a, _b));
      assert(sameTime(_a, _c));
      assert(sameTime(_b, _c));
      assert(differenceInCalendarDays(_c, _b) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_b, _a) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_c, _a) === 2); // 2 normal 24-hour days
    } // TEST DST END (FALL)
...

    {
      ...
      assert(differenceInCalendarDays(_c2, _b2) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_b2, _a2) === 1); // 24.75 hours but 1 calendar days

      assert(differenceInCalendarDays(_c2, _a2) === 2); // 49.75 hours but 2 calendar days
    } // anchor to one hour before the boundary

    {
      var _a3 = new Date(end.getTime() - HOUR); // 1 hour before Standard Time starts
      ...
      assert(sameTime(_a3, _b3));
      assert(sameTime(_a3, _c3));
      assert(sameTime(_b3, _c3));
      assert(differenceInCalendarDays(_c3, _b3) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_b3, _a3) === 1); // 25 hours -> 1 day

      assert(differenceInCalendarDays(_c3, _a3) === 2); // 49 hours -> 2 days
    } // anchor to one hour after the boundary

    {
      var _a4 = new Date(end.getTime() + HOUR); // 1 hour after Standard Time starts
      ...
      assert(sameTime(_a4, _b4));
      assert(sameTime(_a4, _c4));
      assert(sameTime(_b4, _c4));
      assert(differenceInCalendarDays(_c4, _b4) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_b4, _a4) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_c4, _a4) === 2); // 2 normal 24-hour days
    } // anchor exactly at the boundary

    {
      ...
      assert(differenceInCalendarDays(_b5, _a5) === 1); // normal 24-hour day

      assert(differenceInCalendarDays(_c5, _a5) === 2); // 2 normal 24-hour days
    }
  });
});
...
describe('endOfSecond', function () {
  it('returns the date with the time set to the last millisecond before a second ends', function () {
    ...
    _assert.default.deepStrictEqual(result, new Date(2014, 11, 1, 22, 15, 30, 999));
  });
  it('accepts a timestamp', function () {
    var result = (0, _index.default)(new Date(2014, 11, 1, 22, 15, 45).getTime());

    _assert.default.deepStrictEqual(result, new Date(2014, 11, 1, 22, 15, 45, 999));
  });
  it('does not mutate the original date', function () {
    var date = new Date(2014, 11, 1, 22, 15, 15, 300);
    (0, _index.default)(date);

    _assert.default.deepStrictEqual(date, new Date(2014, 11, 1, 22, 15, 15, 300));
  });

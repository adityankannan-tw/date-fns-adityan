...
describe('differenceInQuarters', function () {
  it('returns the number of full quarters between the given dates  with `trunc` as a default rounding method', function () {
    var result = differenceInQuarters(new Date(2012, 6
    /* Jul */
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInQuarters(new Date(2011, 6
    /* Jul */
  ...
  it('returns a 0, not a negative 0 - issue #2555 ', function () {
    var result = differenceInQuarters(new Date(2021, 6
    /* Jul */
  ...
  it('accepts timestamps', function () {
    var result = differenceInQuarters(new Date(2014, 9
    /* Oct */
  ...
  describe('edge cases', function () {
    it('the difference is less than a quarter, but the given dates are in different calendar quarters', function () {
      var result = differenceInQuarters(new Date(2014, 6
      /* Jul */
      , 1), new Date(2014, 5
      /* Jun */
      , 30));
    ...
    it('the same for the swapped dates', function () {
      var result = differenceInQuarters(new Date(2014, 5
      /* Jun */
      , 30), new Date(2014, 6
      /* Jul */
      , 1));
    ...
    it('the days of months of the given dates are the same', function () {
      var result = differenceInQuarters(new Date(2014, 3
      /* Apr */
      , 6), new Date(2014, 0
      /* Jan */
      , 6));
    ...
    it('the given dates are the same', function () {
      var result = differenceInQuarters(new Date(2014, 8
      /* Sep */
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInQuarters(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInQuarters(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });

...
describe('isSameISOWeek', function () {
  it('returns true if the given dates have the same ISO week', function () {
    var result = isSameISOWeek(new Date(2014, 8
    /* Sep */
    , 1), new Date(2014, 8
    /* Sep */
    , 7));
  ...
  it('accepts a timestamp', function () {
    var result = isSameISOWeek(new Date(2014, 5
    /* Jun */
  ...
  it('returns false if the first date is `Invalid Date`', function () {
    var result = isSameISOWeek(new Date(NaN), new Date(1989, 6
    /* Jul */
    , 10));
    assert(result === false);
  });
  it('returns false if the second date is `Invalid Date`', function () {
    var result = isSameISOWeek(new Date(1987, 1
    /* Feb */
    , 11), new Date(NaN));
    assert(result === false);
  });

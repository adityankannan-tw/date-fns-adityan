...
describe('compareAsc', function () {
  it('returns 0 if the given dates are equal', function () {
    var result = compareAsc(new Date(1989, 6
    /* Jul */
    , 10), new Date(1989, 6
    /* Jul */
    , 10));
  ...
  it('returns -1 if the first date is before the second one', function () {
    var result = compareAsc(new Date(1987, 1
    /* Feb */
    , 11), new Date(1989, 6
    /* Jul */
    , 10));
  ...
  it('sorts the dates array in the chronological order when function is passed as the argument to Array.prototype.sort()', function () {
    ...
    /* Jul */
    , 10)];
    var sortedArray = [new Date(1987, 1
    /* Feb */
    , 11), new Date(1989, 6
    /* Jul */
    , 10), new Date(1995, 6
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = compareAsc(new Date(NaN), new Date(1989, 6
    /* Jul */
    , 10));
    assert(isNaN(result));
  });

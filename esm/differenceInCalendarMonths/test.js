...
describe('differenceInCalendarMonths', function () {
  it('returns the number of calendar months between the given dates', function () {
    var result = differenceInCalendarMonths(new Date(2012, 6
    /* Jul */
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInCalendarMonths(new Date(2011, 6
    /* Jul */
  ...
  it('accepts timestamps', function () {
    var result = differenceInCalendarMonths(new Date(2014, 7
    /* Aug */
  ...
  describe('edge cases', function () {
    it('the difference is less than a month, but the given dates are in different calendar months', function () {
      var result = differenceInCalendarMonths(new Date(2014, 8
      /* Sep */
      , 1), new Date(2014, 7
      /* Aug */
      , 31));
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInCalendarMonths(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInCalendarMonths(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });

...
describe('differenceInDays', function () {
  it('returns the number of full days between the given dates', function () {
    var result = differenceInDays(new Date(2012, 6
    /* Jul */
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInDays(new Date(2011, 6
    /* Jul */
  ...
  it('accepts timestamps', function () {
    var result = differenceInDays(new Date(2014, 8
    /* Sep */
  ...
  describe('edge cases', function () {
    ...
    dstOnly("works across DST start & end in local timezone: ".concat(tz || '(unknown)'), function () {
...

      {
        ...
        assert(differenceInDays(_c, _b) === 1); // normal 24-hour day

        assert(differenceInDays(_b, _a) === 1); // normal 24-hour day

        assert(differenceInDays(_c, _a) === 2); // 2 normal 24-hour days
      } // TEST DST END (FALL)
...

      {
        ...
        assert(differenceInDays(_c2, _b2) === 1); // normal 24-hour day

        assert(differenceInDays(_b2, _a2) === 0); // 24.75 hours -> 0 full days (because hour lost to DST)

        assert(differenceInDays(_c2, _a2) === 1); // 48.75 hours -> 1 full day (because hour lost to DST)
      } // anchor to one hour before the boundary

      {
        ...
        assert(differenceInDays(_c3, _b3) === 1); // normal 24-hour day

        assert(differenceInDays(_b3, _a3) === 1); // 25 hours -> 1 day

        assert(differenceInDays(_c3, _a3) === 2); // 49 hours -> 2 days
...

      {
        ...
        assert(differenceInDays(_c4, _b4) === 1); // normal 24-hour day

        assert(differenceInDays(_b4, _a4) === 1); // normal 24-hour day

        assert(differenceInDays(_c4, _a4) === 2); // 2 normal 24-hour days
...

      {
        ...
        assert(differenceInDays(_b5, _a5) === 1); // normal 24-hour day

        assert(differenceInDays(_c5, _a5) === 2); // 2 normal 24-hour days
      }
    });
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInDays(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInDays(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });

...
describe('differenceInCalendarISOWeekYears', function () {
  it('returns the number of calendar ISO week-numbering years between the given dates', function () {
    var result = differenceInCalendarISOWeekYears(new Date(2012, 6
    /* Jul */
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInCalendarISOWeekYears(new Date(2011, 6
    /* Jul */
  ...
  it('accepts timestamps', function () {
    var result = differenceInCalendarISOWeekYears(new Date(2014, 6
    /* Jul */
  ...
  it('handles dates before 100 AD', function () {
    ...
    firstDate.setFullYear(14, 0
    /* Jan */
    , 1);
    firstDate.setHours(0, 0, 0, 0);
    var secondDate = new Date(0);
    secondDate.setFullYear(0, 0
    /* Jan */
    , 1);
  ...
  describe('edge cases', function () {
    it('the difference is less than an ISO year, but the given dates are in different calendar ISO years', function () {
      var result = differenceInCalendarISOWeekYears(new Date(2012, 0
      /* Jan */
      , 2), new Date(2012, 0
      /* Jan */
      , 1));
    ...
    it('the ISO weeks and weekdays of the given dates are the same', function () {
      var result = differenceInCalendarISOWeekYears(new Date(2013, 11
      /* Dec */
      , 30), new Date(2012, 0
      /* Jan */
      , 2));
    ...
    it('the given dates are the same', function () {
      var result = differenceInCalendarISOWeekYears(new Date(2014, 8
      /* Sep */
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInCalendarISOWeekYears(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInCalendarISOWeekYears(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });

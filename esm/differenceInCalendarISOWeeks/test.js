...
describe('differenceInCalendarISOWeeks', function () {
  it('returns the number of calendar ISO weeks between the given dates', function () {
    var result = differenceInCalendarISOWeeks(new Date(2014, 6
    /* Jul */
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInCalendarISOWeeks(new Date(2014, 5
    /* Jun */
  ...
  describe('edge cases', function () {
    ...
    it('the given dates are the same', function () {
      var result = differenceInCalendarISOWeeks(new Date(2014, 8
      /* Sep */
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInCalendarISOWeeks(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInCalendarISOWeeks(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });

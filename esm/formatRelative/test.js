...
describe('formatRelative', function () {
  ...
  it('before the last week', function () {
    var result = formatRelative(new Date(1986, 2
    /* Mar */
    , 28, 16, 50), baseDate);
    assert(result === '03/28/1986');
  });
  it('last week', function () {
    var result = formatRelative(new Date(1986, 3
    /* Apr */
    , 1), baseDate);
  ...
  describe('edge cases', function () {
    it("throws RangeError if the date isn't valid", function () {
      assert.throws(formatRelative.bind(null, new Date(NaN), baseDate), RangeError);
    });
    it("throws RangeError if the base date isn't valid", function () {
      assert.throws(formatRelative.bind(null, new Date(2017, 0
      /* Jan */
      , 1), new Date(NaN)), RangeError);
    });
    it("throws RangeError if both dates aren't valid", function () {
      assert.throws(formatRelative.bind(null, new Date(NaN), new Date(NaN)), RangeError);
    });
  ...
  describe('custom locale', function () {
    ...
    it("throws `RangeError` if `options.locale` doesn't have `localize` property", function () {
      ...
      var block = function block() {
        return formatRelative(new Date(2017, 0, 1), baseDate, {
          // @ts-expect-error
          locale: customLocale
        });
      };

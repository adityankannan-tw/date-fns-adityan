...
describe('differenceInBusinessDays', function () {
  it('returns the number of business days between the given dates, excluding weekends', function () {
    var result = differenceInBusinessDays(new Date(2014, 6
    /* Jul */
    , 18), new Date(2014, 0
    /* Jan */
    , 10));
  ...
  it('can handle long ranges', function () {
    ...
    var result = differenceInBusinessDays(new Date(15000, 0
    /* Jan */
    , 1), new Date(2014, 0
    /* Jan */
    , 1));
  ...
  it('the same except given first date falls on a weekend', function () {
    var result = differenceInBusinessDays(new Date(2019, 6
    /* Jul */
    , 20), new Date(2019, 6
    /* Jul */
    , 18));
  ...
  it('returns a negative number if the time value of the first date is smaller', function () {
    var result = differenceInBusinessDays(new Date(2014, 0
    /* Jan */
    , 10), new Date(2014, 6
    /* Jul */
    , 20));
  ...
  it('accepts timestamps', function () {
    var result = differenceInBusinessDays(new Date(2014, 6, 18).getTime(), new Date(2014, 0, 10).getTime());
    assert(result === 135);
  });
  describe('edge cases', function () {
    it('the difference is less than a day, but the given dates are in different calendar days', function () {
      var result = differenceInBusinessDays(new Date(2014, 8
      /* Sep */
    ...
    it('returns NaN if the first date is `Invalid Date`', function () {
      var result = differenceInBusinessDays(new Date(NaN), new Date(2017, 0
      /* Jan */
      , 1));
      assert(isNaN(result));
    });
    it('returns NaN if the second date is `Invalid Date`', function () {
      var result = differenceInBusinessDays(new Date(2017, 0
      /* Jan */
      , 1), new Date(NaN));
      assert(isNaN(result));
    });

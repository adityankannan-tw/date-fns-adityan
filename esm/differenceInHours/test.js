...
describe('differenceInHours', function () {
  it('returns the number of hours between the given dates with `trunc` as a default rounding method', function () {
    var result = differenceInHours(new Date(2014, 6
    /* Jul */
  ...
  it('returns a 0, not a negative 0 - issue #2555 ', function () {
    var result = differenceInHours(new Date(2021, 6
    /* Jul */
  ...
  it('accepts timestamps', function () {
    var result = differenceInHours(new Date(2014, 8
    /* Sep */
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInHours(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInHours(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });

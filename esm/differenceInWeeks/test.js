...
describe('differenceInWeeks', function () {
  it('returns the number of full weeks between the given dates', function () {
    var result = differenceInWeeks(new Date(2014, 6
    /* Jul */
  ...
  it('returns the number of weeks between the given dates with `trunc` as default a rounding method', function () {
    var result = differenceInWeeks(new Date(2014, 5
    /* Jun */
  ...
  it('returns a 0, not a negative 0 - issue #2555 ', function () {
    var result = differenceInWeeks(new Date(2021, 6
    /* Jul */
  ...
  describe('edge cases', function () {
    ...
    it('the given dates are the same', function () {
      var result = differenceInWeeks(new Date(2014, 8
      /* Sep */
  ...
  it('returns NaN if the first date is `Invalid Date`', function () {
    var result = differenceInWeeks(new Date(NaN), new Date(2017, 0
    /* Jan */
    , 1));
    assert(isNaN(result));
  });
  it('returns NaN if the second date is `Invalid Date`', function () {
    var result = differenceInWeeks(new Date(2017, 0
    /* Jan */
    , 1), new Date(NaN));
    assert(isNaN(result));
  });
